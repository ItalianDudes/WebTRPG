...
public class DND5ESheetTabPrivilegesAndTraits extends AuditableEntity {
    ...
    // Privileges & Traits
    @OneToMany(fetch = FetchType.EAGER, cascade = CascadeType.ALL) @JoinColumn(name = "sheet_id") private List<DND5EClassPrivilege> classPrivileges = new ArrayList<>();
    @OneToMany(fetch = FetchType.EAGER, cascade = CascadeType.ALL) @JoinColumn(name = "sheet_id") private List<DND5ETrait> traits = new ArrayList<>();
