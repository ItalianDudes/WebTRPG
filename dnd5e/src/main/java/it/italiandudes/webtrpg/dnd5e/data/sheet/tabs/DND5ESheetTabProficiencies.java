...
public class DND5ESheetTabProficiencies extends AuditableEntity {
    ...
    // Proficiencies
    @OneToMany(fetch = FetchType.EAGER, cascade = CascadeType.PERSIST) @JoinColumn(name = "sheet_id", nullable = false) private List<DND5EArmorProficiency> armors = new ArrayList<>();
    @OneToMany(fetch = FetchType.EAGER, cascade = CascadeType.PERSIST) @JoinColumn(name = "sheet_id", nullable = false) private List<DND5EWeaponProficiency> weapons = new ArrayList<>();
    @OneToMany(fetch = FetchType.EAGER, cascade = CascadeType.PERSIST) @JoinColumn(name = "sheet_id", nullable = false) private List<DND5EToolProficiency> tools = new ArrayList<>();
    @OneToMany(fetch = FetchType.EAGER, cascade = CascadeType.PERSIST) @JoinColumn(name = "sheet_id", nullable = false) private List<DND5EActivityProficiency> activities = new ArrayList<>();
